...
class DRLRefiner(ReinforceLearnGNN):
    ...
    def A2C_train(self,
        ...
        training_dataset = self._make_coarse_dataset(training_dataset, partitioner=partitioner, **kwargs)
        ...
        for epoch in range(1, epochs+1):
            ...
            for i in range(len(training_dataset)):
                graph, starter, subset_k = self.get_sample(training_dataset[shuffled_ids[i]], partitioner=partitioner, **kwargs)
                ...
                for t in range(len_episode):
                    ...
                    reward = self.reward_function(new_state, graph, new_starter, starter, action, **kwargs)
    ...
    def _make_coarse_dataset(self, dataset: MeshDataset, N: int,
                             partitioner: AgglomerationModel | None = None, threshold: int = 200, **kwargs):
        result = []
        i = 0
    ...
    def get_sample(self, mesh: Mesh | Data, k: int = 3,
        ...
        coarse_graph = avg_pool(cluster.to(DEVICE), Batch(
            batch=graph.batch, x=graph.x, edge_index=graph.edge_index))
        # partition
        cl = partitioner._bisect_subgraph(coarse_graph, np.arange(coarse_graph.num_nodes), 2) # mesh.dim
        one_hot_part = torch.zeros((coarse_graph.num_nodes, 2), dtype=torch.float, device=DEVICE)
    ...
    def _setup_intial_subgraph(self, biparted_graph, g_red, subset_k):
        ...
        g_red.x = torch.cat((
            g_red.x,
            volumes[0]/total_vol*torch.ones((g_red.num_nodes, 1), dtype=torch.float, device=DEVICE),
            volumes[1]/total_vol*torch.ones((g_red.num_nodes, 1), dtype=torch.float, device=DEVICE)), dim=-1)
        return g_red
    ...
    def k_hop_graph_cut(self, graph: Data, k: int):
        ...
        subset_k, edge_ind_k, _, _ = k_hop_subgraph(neib, k, graph.edge_index,
                                                    relabel_nodes=True)
        subset_k_1, _, _, _ = k_hop_subgraph(neib, k - 1, graph.edge_index,
                                             relabel_nodes=True)
        nodes_boundary = set(subset_k.cpu()).difference(subset_k_1.cpu())
        boundary_features = torch.tensor([1 if i in nodes_boundary else 0 for i in subset_k],
                                         dtype=torch.float, device=DEVICE).reshape((-1, 1))
    ...
    def reward_function(self, new_state: Data, old_state: Data,
                        new_starter: Data, old_starter: Data,
                        action: int,
                        disc_penalty=0, cerchiobottismo=0, **kwargs):
        ...
        if (old_state.x[0, -2] > old_state.x[0, -1] and old_state.x[action, 0] == 1
                or old_state.x[0, -2] < old_state.x[0, -1] and old_state.x[action, 0] == 0):
            # if chosen correctly, reward proportional to the imbalance in volumes.
            reward += cerchiobottismo*torch.pow(old_state.x[0, -2] - old_state.x[0, -1], 2)
        else:
            reward -= cerchiobottismo*torch.pow(old_state.x[0, -2] - old_state.x[0, -1], 2)
    ...
    def _refine(self, biparted_graph: Data, k: int = 3, vols=None):
        ...
        old_nc = self.cut(graph)*(1/graph.x[0, -2]+1/graph.x[0, -1])/biparted_graph.num_edges
        ...
        for i in range(len_episode):
            ...
            actions.append(action)
            graph = new_state
            reward = old_nc - self.cut(graph)*(1/graph.x[0, -2]+1/graph.x[0, -1])/biparted_graph.num_edges
...
class Reyyy(DRLRefiner):
    ...
    def forward(self, graph):
        ...
        x_critic = self.act(self.conv_critic(x.detach(), edge_index))  # !!! i put more act
        x_critic = self.final_critic(x_critic)
        x_critic = self.act(global_mean_pool(x_critic, batch))
        return x_actor, x_critic
