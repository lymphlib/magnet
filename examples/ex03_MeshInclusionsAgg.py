...
def create_grid_plot(Meshes: list[list], row_labels, col_labels, labels_fontsize=12, subplot_size=4, **kwargs):
    """
    ...
    fig, axes = plt.subplots(nrows=N, ncols=M, figsize=(M * subplot_size, N * subplot_size))
    ...
    for i, label in enumerate(row_labels):
        fig.text(0.045, (N - i*0.87 - 0.5) / N-0.05, label, va='center', ha='center', 
                 fontsize=labels_fontsize, fontweight='bold', rotation=90)
...
if __name__ == "__main__":
    ...
    for i in range(len(N_holes)):
        ...
        magnet.generate.circular_inclusions('data/'+incl_names[-1], lc[i], N_holes[i], radii[i])

    # Agglomerate
    original_inclusions_meshes = [magnet.io.load_mesh('data/'+name) for name in incl_names]
    ...
    sagehet.load_model('magnet/models/SAGEhetero2D.pt')
    ...
    for i, mesh in enumerate(original_inclusions_meshes):
        ...
        for j, model in enumerate(models):
            if j<2:
                M = model.agglomerate(mesh, 'segregated', mult_factor=1.5*radii[i]/2**0.5)
            else:
                M = model.agglomerate(mesh, 'mult_factor', mult_factor=1.5*radii[i]/2**0.5)
            mesh_to_plot[i].append(M)

    # Plot
    colors = ['#FF7900','#DADBDD']
    col_labels= ['Original mesh','Segregated-METIS','Segregated-SAGE-Base', 'SAGE-Heterogeneous']
    row_labels = ['N=4', 'N=16', 'N=36', 'N=64']
    fig = create_grid_plot(mesh_to_plot, row_labels, col_labels,
                        subplot_size=6, labels_fontsize=18,
                        view_phys=True, palette=colors, edge_color='black', line_width=0.1)
    plt.show()