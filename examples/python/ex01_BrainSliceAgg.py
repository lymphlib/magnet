...
def main(modeldir, meshdir):
    brain_slice = magnet.io.load_mesh(meshdir + "/BrainCoronalNoHoles.vtu")
    color = "#F0F0F0"
    ...
    M_sagecp_rlr = sage.agglomerate(
        brain_slice, "multilevel", nref=7, refiner=rlrefiner
    )
    M_sagecp_rlr.view(
    ...
    M_rlcp_rlr = rlcoarse.agglomerate(
        brain_slice, "multilevel", threshold=120, nref=7, refiner=rlrefiner
    )
    M_rlcp_rlr.view(
